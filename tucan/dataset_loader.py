...
def load_samples_from_source(
    ...
    **kwargs
    ...
    if source_type == "local":
        ...
        return _load_hf_dataset(source, split=split, subset=subset, token=token, **kwargs)
...
def _detect_source_type(source: str) -> str:
    ...
    if "/" in source and any(source.endswith(ext) for ext in [".json", ".jsonl", ".csv", ".parquet"]):
        return "hf_file"
    ...
    try:
        ...
        if isinstance(dataset, Dataset):
            ...
            if split is None:
                ...
                print(f"ℹ️  No split specified, using '{split}' (available: {available_splits})")
...
def _load_hf_file(
    ...
    try:
        ...
        if len(parts) < 3:
            raise ValueError("HuggingFace file path must be in format: username/repo-name/file.json")
...
def list_available_files(repo_id: str, token: Optional[str] = None) -> List[str]:
    ...
    try:
        ...
        data_files = [f for f in files if any(f.endswith(ext) for ext in ['.json', '.jsonl', '.csv', '.parquet'])]
        return data_files
    except Exception as e:
